...
def add_liquidity_features(
    ...
    df['roll_spread_est'] = (2 * np.sqrt((-roll_cov).clip(lower=0)) / c).astype('float32')
    ...
    df['volume_trend_10d'] = (v.rolling(5).mean() / v.rolling(20).mean().clip(lower=1)).astype('float32')

    # Price-volume divergence
    # Price up but volume down = weak rally (bearish divergence)
    price_roc_5 = c.pct_change(5)
    vol_roc_5 = v.pct_change(5)
    ...
    spread_z = (df['hl_spread_proxy'] - df['hl_spread_proxy'].rolling(60).mean()) / df['hl_spread_proxy'].rolling(60).std().clip(lower=1e-8)
    amihud_z = (df['amihud_illiq'] - df['amihud_illiq'].rolling(60).mean()) / df['amihud_illiq'].rolling(60).std().clip(lower=1e-8)
        ...
        try:
            ...
            for col in updated.columns:
                if col not in df.columns:
                    indicators_by_symbol[symbol][col] = updated[col]
